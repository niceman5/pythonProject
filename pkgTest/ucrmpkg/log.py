"""로그 파일 모듈
Log Levels
로그는 아래 목록 중 하나의 레벨을 가지며, 각 레벨의 로그는 각기 다른 용도를
지니고 있다.

DEBUG: 프로그램 작동에 대한 상세한 정보를 가지는 로그. 문제의 원인을 파악할
    때에만 이용.
INFO: 프로그램 작동이 예상대로 진행되고 있는지 트래킹하기 위해 사용.
WARNING: 예상치 못한 일이나 추후 발생 가능한 문제를 표시하기 위함.
ERROR: 심각한 문제로 인해 프로그램이 의도한 기능을 수행하지 못하고 있는 상황을
        표시.
CRITICAL: 더욱 심각한 문제로 인해 프로그램 실행 자체가 언제라도 중단될 수 있음을
표시.
...
def get_logger(name: str = None, log_dir: str = "./logs"):
    """로그 인스턴스를 생성한다.
    Python에서 제공하는 logging을 사용한다.

    Args :
        name (string) : 로그 인스턴스의 이름
        log_dir(string) : 로그가 생성될 위치. Default로 "./logs" 가 된다.
    ...
    # 4 handler instance 생성하여 console 및 파일로 저장할 수 있도록 합니다. 
    # 파일명은 txt도 됩니다.
